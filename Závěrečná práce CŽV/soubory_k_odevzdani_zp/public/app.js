/**
 * SDH Borotín - Klientská část webové aplikace
 * ===========================================
 * Tento soubor obsluhuje uživatelské rozhraní, spouštění kvízů a datovou komunikaci (API) se serverem.
 * Architektura je postavena na principu SPA (Single Page Application). To znamená, že stránka se nenačítá znovu,
 * ale my pouze pomocí JavaScriptu skrytě přepínáme CSS třídy (.hidden-section a .active-section).
 * 
 * KLÍČOVÉ FUNKCE PRO OBHAJOBU:
 * 1. zobrazAplikaci(idSekce) - Jednoduchý CSS "router", přepíná okna.
 * 2. spustitTest(kategorie) - Komunikuje se serverem (API /api/test-otazky), stahuje personalizovaný mix otázek.
 * 3. Fisher-Yates (míchání) - Je nasazeno jak na poli samotných otázek, tak na variantách odpovědí (uživatel nemůže tipovat).
 * 4. aktualizujUI() - Kreslí otázky (DOM manipulace) do HTML struktury.
 */
...
async function spustitTest(kategorie = 'mix') {
    ...
    document.getElementById('nadpisTestu').innerText = (nazvyKategorii[kategorie] || kategorie.toUpperCase());
    ...
    if (chyby.length === 0) {
        ...
        chyby.forEach(ch => {
            ...
            <div class="error-card">
                <h4>${ch.otazka}</h4>
                <p class="wrong">❌ <span>${ch.vybranyText}</span></p>
                <p class="correct">✅ <strong>${ch.spravnyText}</strong></p>
