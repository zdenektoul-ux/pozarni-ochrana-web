...
/**
 * SDH Borotín - Backend a Aplikační rozhraní (Server)
 * ====================================================
 * Zde „žije“ logika celého systému, běží v prostředí Node.js (framework Express.js).
 * Zajišťuje bezpečné REST API rozhraní pro klientskou část (frontend) a obsluhuje databázi (SQLite).
 * 
 * KLÍČOVÉ KOMPONENTY PRO OBHAJOBU:
 * 1. Express Sessions - Každý přihlášený hasič dostane dočasný "občanský průkaz" (session token), kterým se prokazuje API rozhraní.
 * 2. SQLite (db) - Lokální souborová databáze, nepotřebuje velký databázový server. Bohatě dostačuje pro sbor dobrovolných hasičů.
 * 3. Bcrypt - Hesla zásadně ukládáme v šifrovaném hashi (jednosměrná šifra doplněná o "sůl"). Já ani vy nedokážeme přečíst hesla uživatelů!
 * 4. Algoritmus uložení kvízu - /api/test-otazky/:kategorie rozřazuje historii záznamů do skupin (Nové, Kritické, Upevňující, Zvládnuté) přesně podle metody Spaced Repetition (Rozložené opakování).
 */
